...
// Options for the Subgroup dropdown — should mirror Account Sub Group descriptions.
// Replace with a fetch from /api/master/account-sub-group when wiring the real API.
...
// ---------------------------------------------------------------------------
// Seed data (replace with an API call — see fetchAccountAddGroup1() below)
// ---------------------------------------------------------------------------
const SEED_DATA: AccountAddGroup1Row[] = [
  { id: 1, description: "bbca", code: "bb00jjhh", subgroup: "dc" },
  { id: 2, description: "fff", code: "ff00001", subgroup: "dc" },
  { id: 3, description: "musi bhaiiii", code: "mu0000225", subgroup: "sssss" },
];

const PAGE_SIZE = 10;

type SortKey = "id" | "description" | "code" | "subgroup";
type SortDir = "asc" | "desc";

export default function AccountAddGroup1MasterPage() {
  const [rows, setRows] = useState<AccountAddGroup1Row[]>(SEED_DATA);
  ...
  const [editingRow, setEditingRow] = useState<AccountAddGroup1Row | null>(null);
  const [form, setForm] = useState({ description: "", code: "", subgroup: SUBGROUP_OPTIONS[0] });
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      ...
      const subgroupMatch =
        !subgroupFilter || r.subgroup.toLowerCase().includes(subgroupFilter.toLowerCase());
      return globalMatch && descMatch && codeMatch && subgroupMatch;
    });
  }, [rows, search, descFilter, codeFilter, subgroupFilter]);
  ...
  function openAddModal() {
    setEditingRow(null);
    setForm({ description: "", code: "", subgroup: SUBGROUP_OPTIONS[0] });
    setModalOpen(true);
  }

  function openEditModal(row: AccountAddGroup1Row) {
    setEditingRow(row);
    setForm({ description: row.description, code: row.code, subgroup: row.subgroup });
    setModalOpen(true);
  }
  ...
  function saveRow() {
    ...
    if (editingRow) {
      setRows((prev) =>
        prev.map((r) =>
          r.id === editingRow.id
            ? { ...r, description: form.description, code: form.code, subgroup: form.subgroup }
            : r
        )
      );
    } else {
      ...
      setRows((prev) => [
        ...prev,
        { id: nextId, description: form.description, code: form.code, subgroup: form.subgroup },
      ]);
    }
    setModalOpen(false);
  }
  ...
  function copyToClipboard() {
    const text = [
      ["#", "Desc", "Code", "Subgroup"].join("\t"),
      ...sorted.map((r) => [r.id, r.description, r.code, r.subgroup].join("\t")),
    ].join("\n");
  ...
  function exportExcel() {
    ...
    a.href = url;
    a.download = "account-add-group1.csv";
    a.click();
    ...
    <div className="min-h-screen bg-gray-50 p-1">
      <div className="mx-auto max-w-1xl overflow-hidden rounded border border-gray-200 bg-white shadow-sm">
        ...
        <div className="flex items-center justify-between border-t-4 border-t-sky-400 bg-gray-100 px-4 py-2">
          <h1 className="text-sm font-medium text-gray-700">Account Group1</h1>
          <button
              ...
              <thead>
                <tr className="border-b border-gray-200 text-left">
                  ...
                  <th
                    ...
                    className="cursor-pointer select-none py-2 pr-2 font-semibold text-gray-700"
                  >
                    Subgroup <span className="text-gray-400">{sortIndicator("subgroup")}</span>
                  </th>
                </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm rounded bg-white p-4 shadow-lg">
            <h2 className="mb-3 text-sm font-semibold text-gray-800">
              {editingRow ? "Edit Account Group1" : "Add Account Group1"}
            </h2>
            <div className="space-y-3">
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-gray-600">Subgroup</label>
                <select
                  value={form.subgroup}
                  onChange={(e) => setForm((f) => ({ ...f, subgroup: e.target.value }))}
                  className="w-full rounded border border-gray-300 px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-sky-400"
                >
                  {SUBGROUP_OPTIONS.map((sg) => (
...
// ---------------------------------------------------------------------------
// Optional: swap SEED_DATA and SUBGROUP_OPTIONS for real API calls, e.g.
//
// async function fetchAccountAddGroup1(): Promise<AccountAddGroup1Row[]> {
//   const res = await fetch("/api/master/account-add-group1");
//   return res.json();
// }
//
// async function fetchSubgroupOptions(): Promise<string[]> {
//   const res = await fetch("/api/master/account-sub-group");
//   const subgroups = await res.json();
//   return subgroups.map((s: { subGroupDesc: string }) => s.subGroupDesc);
// }
//
// then load both in a useEffect and setRows(data) / setSubgroupOptions(data).
// ---------------------------------------------------------------------------