...
// Options for the Group dropdown — should mirror Account Group descriptions.
...
// ---------------------------------------------------------------------------
// Seed data (replace with an API call — see fetchAccountSubGroups() below)
// ---------------------------------------------------------------------------
const SEED_DATA: AccountSubGroupRow[] = [
  { id: 1, subGroupDesc: "bca", code: "bc00001", group: "A" },
  { id: 2, subGroupDesc: "Beer", code: "Be00001", group: "Current Assets" },
  { id: 3, subGroupDesc: "dc", code: "dc00001", group: "abc" },
  { id: 4, subGroupDesc: "egro", code: "eg00001", group: "random" },
  { id: 5, subGroupDesc: "eqrqgt", code: "eq00001", group: "Non-Current Assets" },
  { id: 6, subGroupDesc: "qwd", code: "qw00001", group: "Finance Cost" },
  { id: 7, subGroupDesc: "qwerfdscxz", code: "qw00002", group: "Resurve & Surplus" },
  { id: 8, subGroupDesc: "sddd", code: "sd00001", group: "Expenditure" },
  { id: 9, subGroupDesc: "sssss", code: "ss00001", group: "Current Assets" },
  { id: 10, subGroupDesc: "ytujnk", code: "yt00001", group: "Expenditure" },
];

const PAGE_SIZE = 10;

type SortKey = "id" | "subGroupDesc" | "code" | "group";
type SortDir = "asc" | "desc";

export default function AccountSubGroupMasterPage() {
  const [rows, setRows] = useState<AccountSubGroupRow[]>(SEED_DATA);
  ...
  const [editingRow, setEditingRow] = useState<AccountSubGroupRow | null>(null);
  const [form, setForm] = useState({ subGroupDesc: "", code: "", group: GROUP_OPTIONS[0] });
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      const globalMatch =
        !search ||
        r.subGroupDesc.toLowerCase().includes(search.toLowerCase()) ||
        r.code.toLowerCase().includes(search.toLowerCase()) ||
        r.group.toLowerCase().includes(search.toLowerCase());
      const descMatch = !descFilter || r.subGroupDesc.toLowerCase().includes(descFilter.toLowerCase());
      ...
      return globalMatch && descMatch && codeMatch && groupMatch;
    });
  }, [rows, search, descFilter, codeFilter, groupFilter]);
  ...
  function openAddModal() {
    setEditingRow(null);
    setForm({ subGroupDesc: "", code: "", group: GROUP_OPTIONS[0] });
    setModalOpen(true);
  }

  function openEditModal(row: AccountSubGroupRow) {
    setEditingRow(row);
    setForm({ subGroupDesc: row.subGroupDesc, code: row.code, group: row.group });
    setModalOpen(true);
  }

  function saveRow() {
    if (!form.subGroupDesc.trim() || !form.code.trim()) return;

    if (editingRow) {
      setRows((prev) =>
        prev.map((r) =>
          r.id === editingRow.id
            ? { ...r, subGroupDesc: form.subGroupDesc, code: form.code, group: form.group }
            : r
        )
      );
    } else {
      ...
      setRows((prev) => [
        ...prev,
        { id: nextId, subGroupDesc: form.subGroupDesc, code: form.code, group: form.group },
      ]);
    }
    setModalOpen(false);
  }
  ...
  function copyToClipboard() {
    const text = [
      ["#", "Sub Group Desc", "Code", "Group"].join("\t"),
      ...sorted.map((r) => [r.id, r.subGroupDesc, r.code, r.group].join("\t")),
    ].join("\n");
  ...
  function exportExcel() {
    ...
    a.href = url;
    a.download = "account-sub-group.csv";
    a.click();
    ...
    <div className="min-h-screen bg-gray-50 p-1">
      <div className="mx-auto max-w-1xl overflow-hidden rounded border border-gray-200 bg-white shadow-sm">
        ...
        <div className="flex items-center justify-between border-t-4 border-t-sky-400 bg-gray-100 px-4 py-2">
          <h1 className="text-sm font-medium text-gray-700">Account Sub Group</h1>
          <button
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="border-b border-gray-200 text-left">
                  ...
                  <th
                    onClick={() => toggleSort("subGroupDesc")}
                    className="cursor-pointer select-none py-2 pr-2 font-semibold text-gray-700"
                  >
                    Sub Group Desc <span className="text-gray-400">{sortIndicator("subGroupDesc")}</span>
                  </th>
                  <th
                ...
                {pageRows.length === 0 ? (
                  ...
                  pageRows.map((row) => (
                    <tr key={row.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-2 pr-2 text-gray-700">{row.subGroupDesc}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm rounded bg-white p-4 shadow-lg">
            <h2 className="mb-3 text-sm font-semibold text-gray-800">
              {editingRow ? "Edit Account Sub Group" : "Add Account Sub Group"}
            </h2>
            <div className="space-y-3">
              <div>
                <label className="mb-1 block text-xs font-medium text-gray-600">Sub Group Desc</label>
                <input
                  value={form.subGroupDesc}
                  onChange={(e) => setForm((f) => ({ ...f, subGroupDesc: e.target.value }))}
...
// ---------------------------------------------------------------------------
// Optional: swap SEED_DATA and GROUP_OPTIONS for real API calls, e.g.
//
// async function fetchAccountSubGroups(): Promise<AccountSubGroupRow[]> {
//   const res = await fetch("/api/master/account-sub-group");
//   return res.json();
// }
//
// async function fetchAccountGroupOptions(): Promise<string[]> {
//   const res = await fetch("/api/master/account-group");
//   const groups = await res.json();
//   return groups.map((g: { description: string }) => g.description);
// }
//
// then load both in a useEffect and setRows(data) / setGroupOptions(data).
// ---------------------------------------------------------------------------